...
export function buildTiers(
  ...
  // Starter = ONLY foundational (CORE) items. No mandated/recommended.
  const starter = dedup(unique.filter(s => s.status === 'foundational'))

  // Standard = foundational + mandated + recommended (capped by size).
  const standard = dedup([...foundationalMandated, ...recommended]).slice(0, cap)
