    ...
    <div ref={containerRef} className="relative">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[11px] text-gray-500">
        ...
        <span className="inline-flex items-center gap-1.5">
          ...
          <span><b className="text-[#B45309]">Required</b> · mandated by regulations</span>
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="inline-block w-2.5 h-2.5 rounded-sm bg-sky-50 border border-sky-500" />
          <span><b className="text-[#0369A1]">Recommended</b> · matched to your concerns</span>
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="inline-block w-2.5 h-2.5 rounded-sm border-2 border-dashed border-gray-300" />
          <span><b className="text-gray-500">Future-state</b> · becomes relevant as you scale</span>
        </span>
        <span className="text-gray-400 italic">Hover any module for details.</span>
      </div>

      {/* Compliance Legend */}
      {showCompliance && (
        <div className="mt-4 pt-4 border-t border-[#F0F0F0] flex flex-wrap gap-3 justify-center">
...
function ModuleChip({
  ...
  const chipClass =
    isFuture
      ...
      ? 'border-[#E40000] bg-[#FFE5E5] shadow-sm hover:shadow-md'
      : cap.status === 'mandated'
      ? 'border-amber-500 bg-amber-50 shadow-sm hover:shadow-md'
      : 'border-sky-500 bg-sky-50 hover:shadow-md'

  const badge =
    cap.status === 'foundational'
      ? { label: 'CORE', cls: 'bg-[#E40000]' }
      : cap.status === 'mandated'
      ? { label: 'REQ', cls: 'bg-amber-500' }
      : null
  ...
  return (
    <div
      ...
      className={`relative px-3 py-2 rounded-lg border-2 cursor-default select-none transition-all duration-150 group ${chipClass}`}
    >
      <span className={`text-xs font-semibold ${isFuture ? 'text-gray-400' : 'text-[#1A1A1A]'}`}>
        {cap.name}
      </span>
      {badge && !isFuture && (
        <span className={`absolute -top-1.5 -right-1 ${badge.cls} text-white text-[9px] font-bold px-1 py-0.5 rounded`}>
          {badge.label}
        </span>
      )}
...
const STATUS_INFO: Record<string, { label: string; tone: string; bg: string; explain: string }> = {
  ...
  mandated: {
    ...
    tone: 'text-[#B45309]',
    bg: 'bg-amber-50',
    explain: 'Mandated by a regulation that applies to your industry.',
  },
  recommended: {
    label: 'Recommended',
    tone: 'text-[#0369A1]',
    bg: 'bg-sky-50',
    explain: 'Matched against your concerns and environment.',
  },
}
    ...
    >
      <div className="w-72 bg-white border border-[#E5E5E5] rounded-xl shadow-xl p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          {STATUS_INFO[cap.status] && (
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${STATUS_INFO[cap.status].tone} ${STATUS_INFO[cap.status].bg}`}>
              {STATUS_INFO[cap.status].label}
            </span>
          )}
        </div>
        {STATUS_INFO[cap.status] && (
          <p className="text-[11px] text-gray-500 italic mb-2">
            {STATUS_INFO[cap.status].explain}
          </p>
        )}
